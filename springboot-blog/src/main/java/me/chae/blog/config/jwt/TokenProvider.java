...
public class TokenProvider {
	...
	private String makeToken(Date expiry, User user) {
		...
		return Jwts.builder()
				   ...
				   .setIssuer(jwtProperties.getIssuer())			// 내용 iss(발급자): c@j.a(properties 파일에서 설정한 값)
	...
	public Authentication getAuthentication(String token) {	// 토큰 기반으로 인증 정보를 가져오는 메서드
		...
		// 리턴시 첫번째 인자는 만든 User 클래스가 아니라 스프링 시큐리티에서 제공하는 User클래스로 임포트할 것
		return new UsernamePasswordAuthenticationToken(new org.springframework.security.core.userdetails.User(claims.getSubject(), "", authorities), token, authorities);
	}
	
	public Long getUserId(String token) {	// 토큰 기반으로 유저 ID 를 가져오는 메서드
		Claims claims = getClaims(token);
